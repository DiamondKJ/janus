# /JANUS-CORE/run_janus_interactive.py
...
# Use a try-except block for robustness in case training hasn't been run
try:
    from janus.training.train_arbiter import ArbiterEngine
except ImportError:
    print("WARNING: Could not import ArbiterEngine. Run the --full-pipeline in run_janus.py first.")
    ArbiterEngine = None
...
class JanusV3_Harness:
    ...
    def _load_all_modules(self):
        logger.info("Loading cognitive architecture: Both Hemispheres and the Arbiter...")
        ...
        if ArbiterEngine and os.path.exists(arbiter_path):
            self.arbiter = ArbiterEngine(vocab_size=self.tokenizer.vocab_size, embedding_dim=256, hidden_dim=512, output_dim=1).to(self.device)
            ...
            self.arbiter = None
            logger.warning("Arbiter model not found or could not be loaded. Running in simple blend mode.")
    ...
    def generate(self, prompt, temperature=0.7):
        ...
        if self.arbiter:
            ...
            logger.info(f"Arbiter has set average blending weight to: {blending_weight:.2f}")
        else:
            blending_weight = 0.5 # Default if arbiter is missing
            logger.info("Defaulting to 0.5 blending weight.")
...
def main():
    parser = argparse.ArgumentParser(description="Run the Janus v3.1 Interactive Harness.")
